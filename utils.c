...
int dog_find_path(const char *sef_path, const char *sef_name, const char *ignore_dir)
{
    ...
    if (sef_path == NULL || sef_name == NULL) {
...
dog_configure_toml(void)
{
	...
	toml_table_t	*dog_toml_parse = NULL;
	toml_table_t	*dog_toml_depends = NULL, *dog_toml_compiler = NULL, *general_table = NULL;
	...
	if (dog_toml_compiler != NULL) {
        ...
        if (toml_include_path != NULL) {
            ...
            for (int i = 0; i < toml_array_size; i++) {
                toml_datum_t _toml_path_val = toml_string_at(toml_include_path, i);
        ...
        toml_array_t *option_arr = toml_array_in(dog_toml_compiler, "option");
...
skip_:
        if (option_arr != NULL) {
            ...
            for (int i = 0; i < toml_array_size; i++) {
                toml_datum_t toml_option_value = toml_string_at(option_arr, i);
                ...
                if (strlen(toml_option_value.u.s) >= 2) {
                    snprintf(appended_flags, sizeof(appended_flags), "%.2s", toml_option_value.u.s);
                } else {
                    strncpy(appended_flags, toml_option_value.u.s, sizeof(appended_flags) - 1);
                } /* if */

                if (strfind(toml_option_value.u.s, "-d", true) || pctx->flag_debug > 0) {
                ...
                if (!old_len) {
                    (void)snprintf(expect, new_len, "%s", toml_option_value.u.s);
                } else {
                    (void)snprintf(expect + old_len, new_len - old_len, " %s", toml_option_value.u.s);
                } /* if */
...
skip_compiler:
	...
	for (size_t i = 0; i < sizeof(toml_char_field) / sizeof(toml_char_field[0]); i++) {
	...
	if (strcmp(dogconfig.dog_toml_os_type, OS_SIGNAL_WINDOWS) == 0) {
        ...
        pr_warning(stdout, "dog_configure_toml: unknown OS type %s", dogconfig.dog_toml_os_type);
    } /* if */
	...
	if (ret) {
        pbuf[0] = '\0';
		(void)snprintf(pbuf, sizeof(pbuf), "%s", dogconfig.dog_sef_found_list[0]);
