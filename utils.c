...
#ifdef DOG_WINDOWS
  static win_stupinfo win_stupinfo = {0},
    PROCESS_INFORMATION win_procinfo = {0},
    SECURITY_ATTRIBUTES win_attr = {0};
#endif
...
is_running_in_container(void)
{
  ...
  while (fgets(tempbf, sizeof(tempbf), fp) != NULL) {
    if (strstr(tempbf, "/docker/") ||
      strstr(tempbf, "/podman/") ||
      strstr(tempbf, "/containerd/"))
    {
      fclose(fp);
      return (true);
    } /* if */
...
same_file(const char *a, const char *b)
{
  ...
  if (a == NULL || b == NULL) {
    pr_error(stdout, "same_file: NULL argument");
    return (0);
  } /* if */

  if (stat(a, &sa) != 0) {
        pr_info(stdout, "same_file: stat failed for %s", a);
    return (0);
  } /* if */
  
  if (stat(b, &sb) != 0) {
        pr_info(stdout, "same_file: stat failed for %s", b);
    return (0);
  } /* if */
...
char * procure_pwd(void) {
  ...
  #ifdef DOG_WINDOWS
  if (_getcwd(dog_work_dir, sizeof(dog_work_dir)) == NULL) {
  #else
  if (getcwd(dog_work_dir, sizeof(dog_work_dir)) == NULL) {
  #endif
    pr_error(stdout, "procure_pwd: getcwd failed: %s", strerror(errno));
    dog_work_dir[0] = '\0';
  } /* if */
...
int __MKDIR(const char *path)
{
  ...
  if (!path || !*path) {
    pr_error(stdout, "__MKDIR: invalid path");
    return (-1);
  } /* if */

  char tempbf[DOG_PATH_MAX];

  (void)snprintf(tempbf, sizeof(tempbf), "%s", path);
  len = strlen(tempbf);

  if (len > 1 && tempbf[len - 1] == '/') {
    tempbf[len - 1] = '\0';
  } /* if */
  ...
  for (p = tempbf + 1; *p; p++) {
    if (*p == '/') {
      *p = '\0';
      if (MKDIR(tempbf) != 0 && errno != EEXIST) {
        perror("mkdir");
        return (-1);
      } /* if */
      *p = '/';
...
void print_restore_color() {

  fputs(BKG_DEFAULT, stdout);
  fputs(LR_RESET, stdout);
  fputs(LR_DEFAULT, stdout);
...
{
  if (path == NULL) {
    pr_error(stdout, "print_file: path is NULL");
    return;
  } /* if */
  ...
  if (fd < 0) {
    pr_error(stdout,
      "print_file: failed to open %s", path);
    return;
  } /* if */
  ...
  fd = open(path, O_RDONLY);
  if (fd < 0) {
    pr_error(stdout, "print_file: failed to open %s", path);
    return;
  } /* if */

  if (fstat(fd, &st) < 0) {
    pr_error(stdout, "print_file: fstat failed for %s", path);
    close(fd);
    return;
  } /* if */
...
_Bool console_title(const char *title)
{
  ...
  if (!ok) {
    pr_info(stdout, "console_title: SetConsoleTitleA failed");
  } /* if */
#else
...
_Bool _strend(const char *str, const char *suffix, _Bool nocase)
{
  ...
  if (!str || !suffix) {
    pr_error(stdout, "_strend: NULL argument");
    return (false);
  } /* if */
...
_Bool _strfind(const char *text, const char *pattern, _Bool nocase)
{
  ...
  if (!text || !pattern) {
        pr_error(stdout, "_strfind: NULL argument");
    return (false);
  } /* if */
...
static void configure_path_sep(char *out, size_t size_out,
                               const char *open_dir,
                               const char *entry_name)
{
    ...
    if (!out || size_out == 0 || !open_dir || !entry_name) {
    ...
    char *path = NULL;
    #ifdef DOG_LINUX
      path = _PATH_STR_SEP_POSIX;
    #else
      path = _PATH_STR_SEP_WIN32;
    #endif
        out[pos++] = path[0];
...
static const char * dog_procure_os(void)
{
  ...
  static const char *PROC_WSL_INTEROP_PATH = "/proc/sys/fs/binfmt_misc/WSLInterop";
...
__PURE__
static int
ensure_parent_dir(char *parent, size_t n, const char *dest)
{
...
_Bool
terminate_proc(const char *process)
{
  if (!process) {
    pr_error(stdout, "terminate_proc: process name is NULL");
    return (false);
  } /* if */

#ifdef DOG_WINDOWS
  char tempbf[DOG_PATH_MAX + 128];

  memset(&win_stupinfo, 0, sizeof(&win_stupinfo));
  memset(&win_procinfo, 0, sizeof(&win_procinfo));

  win_stupinfo.cb = sizeof(win_stupinfo);

  const char *command = "C:\\Windows\\System32\\taskkill.exe /F /IM \"%s\"";
    (void)snprintf(tempbf, sizeof(tempbf), command, process);
  (void)CreateProcessA(NULL, tempbf, NULL, NULL, FALSE,
  CREATE_NO_WINDOW, NULL, NULL, &win_stupinfo, &win_procinfo);

  WaitForSingleObject(win_procinfo.hProcess, INFINITE);
  CloseHandle(win_procinfo.hProcess);
  CloseHandle(win_procinfo.hThread);
  ...
  if (process[0] == '.') {
    ...
    if (pid < 0) {
        pr_error(stdout, "terminate_proc: fork failed");
        return (false);
    } /* if */
...
int
dot_or_dotdot(const char *entry_name)
{
  int result = 0;
...
int find_path(const char *sef_path, const char *sef_name, const char *ignore_dir)
{
  char     size_path[MAX_SEF_PATH_SIZE] = {0};
  int          found = 0;
  
  if (strlen(sef_path) < 1 && strlen(sef_name) < 1) {
    pr_error(stdout, "find_path: invalid parameters");
    return 0;
  } /* if */
  ...
  if (find_handle == INVALID_HANDLE_VALUE) {
    pr_info(stdout, "find_path: FindFirstFile failed for %s", sp);
    return (0);
  } /* if */

  do {
    entry_name = find_data.cFileName;
    if (dot_or_dotdot(entry_name)) {
      continue;
    } /* if */
    ...
    if (find_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
      ...
      if (find_path(size_path, sef_name, ignore_dir)) {
  ...
  DIR *dir = NULL;
    ...
    if (!dir) {
        pr_info(stdout, "find_path: opendir failed for %s", sef_path);
        return 0;
    } /* if */
...
_run_windows_command(const char *cmds)
{
  ...
  win_stupinfo.cb = sizeof(win_stupinfo);
  (void)CreateProcess(NULL, (char *)cmds,
    NULL, NULL, FALSE, 0, NULL, NULL, &win_stupinfo, &win_procinfo);

  WaitForSingleObject(win_procinfo.hProcess, INFINITE);
  GetExitCodeProcess(win_procinfo.hProcess, &exit_code);
...
validate_src_dest(const char *c_src, const char *c_dest)
{
  ...
  if (path_exists(c_dest) && same_file(c_src, c_dest)) {
  ...
  if (ensure_parent_dir(tempbf, sizeof(tempbf), c_dest) != 0) {
    ...
    int super_mode)
{
  ...
  char tempbf[DOG_PATH_MAX * 2 + 128];

  if (strcmp(operation, "mv") == 0) {
      (void)snprintf(tempbf, sizeof(tempbf),
  ...
  ret = _run_windows_command(tempbf);
  if (ret > 0) {
    if (strcmp(operation, "mv") == 0) {
      (void)snprintf(tempbf,
      sizeof(tempbf),
...
int
wmove(const char *c_src, const char *c_dest)
{
  int super_mode = 0;
  int ret = -1;
  ...
  if (_strfind(c_dest, "/usr/", true) == false) {
    goto super_mode_check_done;
  } /* if */
...
super_mode_check_done:
    ...
    if (super_mode == 1) {
        ...
        "'" LR_YELLOW "%s" LR_DEFAULT
        "' to '" LR_YELLOW "%s" LR_DEFAULT "'", c_src, c_dest);
    } else if (super_mode == 2) {
      pr_info(stdout,
...
int
wcopy(const char *c_src, const char *c_dest)
{
    int super_mode = 0;
    int ret = -1;
...
dog_check_pc_options(int *compatibility, int *optimized_lt)
{
  ...
  win_attr.nLength = sizeof(win_attr);
  win_attr.bInheritHandle = TRUE;
  ...
  char tempbf[DOG_PATH_MAX + 26];

  if (hFile != INVALID_HANDLE_VALUE) {
    win_stupinfo.cb         = sizeof(win_stupinfo);
    win_stupinfo.dwFlags    = STARTF_USESTDHANDLES;
    win_stupinfo.hStdOutput = hFile;
    win_stupinfo.hStdError  = hFile;
  ...
  if (fpile != NULL) {
    while (fgets(tempbf, sizeof(tempbf), fpile) != NULL) {
      if (!found_Z && _strfind(tempbf, "-Z", true)) {
        found_Z = 1;
            } /* if */
      if (!found_ver && _strfind(tempbf, "3.10.11", true)) {
        found_ver = 1;
            } /* if */
      if (_strfind(tempbf, "error while loading shared libraries:", true) ||
        _strfind(tempbf, "required file not found", true)) {
        print_file(".watchdogs/pawnc_test.log");
      } /* if */
    } /* while */
...
dog_parse_toml_config(void)
{
  ...
  dog_toml_parse = toml_parse_file(fpile, tempbf, sizeof(tempbf));
  fclose(fpile);

  if (!dog_toml_parse) {
    pr_error(stdout, "Parsing TOML: %s", tempbf);
    minimal_debugging();
    return (0);
  } /* if */
...
dog_find_compiler(const char *dog_os_type)
{
  ...
  if (fet_server_env() == false) {
    found = find_path("pawno", pc_name, NULL);
    } else if (fet_server_env() == true) {
    found = find_path("qawno", pc_name, NULL);
    } else {
    found = find_path("pawno", pc_name, NULL);
    } /* if */
...
static _Bool  samp_server_stat = false;
static void
    ...
    int has_gamemodes, int compatible, int optimized_lt, char *sef_path)
{
  ...
  if (compatible && optimized_lt) {
    if (samp_server_stat == true) {
      fprintf(file,
        "   option = [\"-Z:+\", \"-d:2\", \"-O:2\", "
        "\"-;+\", \"-(+\", \"LOCALHOST=1\", 'SERVER=\"Never DM\"'] # compiler options\n");
    } else {
      fprintf(file,
        "   option = [\"-Z:+\", \"-d:2\", \"-O:1\", "
        "\"-;+\", \"-(+\", \"LOCALHOST=1\", 'SERVER=\"Never DM\"'] # compiler options\n");
  ...
  if (has_gamemodes && sef_path[0] != '\0') {
    ...
    if (path_exists("hellonworld.pwn") == 1) {
      fprintf(file,
          "   input = \"hellonworld.pwn\" # project input\n");
      fprintf(file,
          "   output = \"hellonworld.amx\" # project output\n");
    } else {
      fprintf(file,
...
configure_toml(void)
{
  ...
  toml_table_t    *general_table = NULL;
  toml_datum_t     input_val, output_val;
  toml_datum_t     bin_val, conf_val, logs_val;
  ...
  if (toml_file != NULL) {
    ...
    if (!find_pawncc) {
      if (strcmp(dog_os_type, "windows") == 0) {
        find_pawncc = find_path(".", "pawncc.exe", NULL);
      } else {
        find_pawncc = find_path(".", "pawncc", NULL);
      } /* if */
    } /* if */

    find_gamemodes = find_path("gamemodes/", "*.pwn", NULL);
  ...
  dog_toml_parse = toml_parse_file(fp, tempbf, sizeof(tempbf));
  if (fp) {
    fclose(fp);
  } /* if */

  if (!dog_toml_parse) {
    pr_error(stdout, "failed to parse the watchdogs.toml...: %s", tempbf);
    minimal_debugging();
    unit_ret_main(NULL);
  } /* if */
...
skip_:
  ...
  for (i = 0; i < toml_array_size; i++) {
    ...
    if (_strfind(toml_option_value.u.s, "-d", true) ||
      pctx->flag_debug > 0) {
      pc_debug_options = true;
    } /* if */

    old_len = expect ? strlen(expect) : 0;
...
skip_general_tb:
  ...
  for (i = 0; i < sizeof(toml_char_field) / sizeof(toml_char_field[0]); i++) {
...
#ifdef DOG_LINUX
    if (_strfind(field_name, "dog_toml_os_type", 1) == 1) {
    ...
    if (_strfind(field_name, "dog_toml_server_binary", 1) == 1) {
      dogconfig.dog_toml_server_binary = strdup("samp03svr");
    } /* if */

    if (_strfind(field_name, "dog_toml_server_config", 1) == 1) {
      dogconfig.dog_toml_server_config = strdup("server.cfg");
    } /* if */

    if (_strfind(field_name, "dog_toml_server_logs", 1) == 1) {
      dogconfig.dog_toml_server_logs = strdup("server_log.txt");
    } /* if */

    if (_strfind(field_name, "dog_toml_full_opt", 1) == 1) {
      dogconfig.dog_toml_full_opt = strdup("-Z+");
    } /* if */

    if (_strfind(field_name, "dog_toml_serv_input", 1) == 1) {
      dogconfig.dog_toml_serv_input = strdup("none.pwn");
    } /* if */

    if (_strfind(field_name, "dog_toml_serv_output", 1) == 1) {
  ...
  if (ret) {
    (void)snprintf(tempbf, sizeof(tempbf), "%s",
        dogconfig.dog_sef_found_list[0]);
    dogconfig.dog_pawncc_path = strdup(tempbf);
  } else {
    pr_info(stdout,
