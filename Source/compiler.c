    ...
    const char *nine_arg, const char *ten_arg)
{
	...
	const 	char  *posix_fzf_path[] = {
			...
			/* ../storage/emulated/0/Download/folders/files */
		...
		if (*new_compile_args_val == '\0' ||
			...
			if (new_compile_args_val[0] != '.')
			{
				...
				if (!one_show) {
					...
					if (!tree_ret) {
						if (path_exists("../storage/emulated/0/Download")) {
							char *tree[] = {
								"tree", "-P",
								"\"*.p\"", "-P",
								"\"*.pawn\"", "-P",
								"\"*.pwn\"", "../storage/emulated/0/Download",
								NULL
							};
						...
						#ifdef DOG_LINUX
						if (path_exists("../storage/emulated/0/Download") == 1) {
							char *argv[] = { "ls", "../storage/emulated/0/Download", "-R", NULL };
