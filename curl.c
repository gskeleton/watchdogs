  ...
  int* found_pawnc_dll, int* found_PAWNC_DLL, int* found_pawnruns, int* found_pawnruns_exe)
{
  ...
  *found_pawncc_exe = find_path(pawncc_dir_source, "pawncc.exe", ignore_dir);
  *found_pawncc = find_path(pawncc_dir_source, "pawncc", ignore_dir);
  *found_pawndisasm_exe = find_path(pawncc_dir_source, "pawndisasm.exe", ignore_dir);
  *found_pawndisasm = find_path(pawncc_dir_source, "pawndisasm", ignore_dir);
  *found_PAWNC_DLL = find_path(pawncc_dir_source, "PAWNC.dll", ignore_dir);
  *found_pawnc_dll = find_path(pawncc_dir_source, "pawnc.dll", ignore_dir);
  *found_pawnruns = find_path(pawncc_dir_source, "pawnruns", ignore_dir);
  *found_pawnruns_exe = find_path(pawncc_dir_source, "pawnruns.exe", ignore_dir);
  ...
  if (*found_pawncc_exe < 1 && *found_pawncc < 1) {
    ...
    *found_pawncc_exe = find_path(".", "pawncc.exe", ignore_dir);
    *found_pawncc = find_path(".", "pawncc", ignore_dir);
    *found_PAWNC_DLL = find_path(".", "PAWNC.dll", ignore_dir);
    *found_pawnc_dll = find_path(".", "pawnc.dll", ignore_dir);
    *found_pawndisasm_exe = find_path(".", "pawndisasm.exe", ignore_dir);
    *found_pawndisasm = find_path(".", "pawndisasm", ignore_dir);
  ...
  if (*found_pawncc_exe < 1 && *found_pawncc < 1) {
    ...
    *found_pawncc_exe = find_path("bin/", "pawncc.exe", ignore_dir);
    *found_pawncc = find_path("bin/", "pawncc", ignore_dir);
    *found_PAWNC_DLL = find_path("bin/", "PAWNC.dll", ignore_dir);
    *found_pawnc_dll = find_path("bin/", "pawnc.dll", ignore_dir);
    *found_pawndisasm_exe = find_path("bin/", "pawndisasm.exe", ignore_dir);
    *found_pawndisasm = find_path("bin/", "pawndisasm", ignore_dir);
...
static int setup_linux_library(void)
{
  ...
  /* Find libpawnc.so */
  found_lib = find_path(pawncc_dir_source, "libpawnc.so", NULL);

  if (found_lib < 1) {
    found_lib = find_path(".", "libpawnc.so", NULL);
    if (found_lib < 1) {
      found_lib = find_path("lib/", "libpawnc.so", NULL);
    } /* if */
  } /* if */
  ...
  if (path_exists(libpawnc_src) == 1)
  {
    ...
    if (!na_hexdump) {
      print_restore_color();
      pr_info(stdout,
        "Fetching " LR_YELLOW "%s " LR_DEFAULT "binary hex..", libpawnc_src);
  ...
  char* data, size_t size, void* userptr __UNUSED__)
{
  ...
  case CURLINFO_HEADER_IN:
    ...
    if (_strfind(data, "content-security-policy: ", true)) {
      break;
    } /* if */
...
dog_download_file(const char* url, const char* output_filename)
{
  ...
  pr_color(stdout, LR_GREEN, "* Try Downloading %s", curl_url_finalname);
  ...
  while (retry_count < max_retries) {
    ...
    static _Bool rate_create_debugging = false;
