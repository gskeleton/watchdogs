...
curl_verify_cacert_pem(CURL* curl)
{
	...
	pr_info(stdout, "curl_verify_cacert_pem: platform = %d", platform_specific);

	/* Handle based on platform */
	if (platform_specific == 3) {
		/* Windows: check local and system paths */
		pr_info(stdout, "curl_verify_cacert_pem: Windows platform");
		
		if (path_access("cacert.pem") != 0) {
			curl_easy_setopt(curl, CURLOPT_CAINFO, "cacert.pem");
			pr_info(stdout, "curl_verify_cacert_pem: using local cacert.pem");
		} else if (access("C:/libdog/cacert.pem", F_OK) == 0) {
			curl_easy_setopt(curl, CURLOPT_CAINFO,
				"C:/libdog/cacert.pem");
			pr_info(stdout, "curl_verify_cacert_pem: using system cacert.pem");
		} else {
		...
		pr_info(stdout, "curl_verify_cacert_pem: Android/Termux platform");
		
		if (!prefix || prefix[0] == '\0') {
			prefix = "/data/data/com.termux/files/usr";
			pr_info(stdout, "curl_verify_cacert_pem: using default prefix: %s", prefix);
		} /* if */
		...
		if (access(ca1, F_OK) == 0) {
			...
			pr_info(stdout, "curl_verify_cacert_pem: using %s", ca1);
		} else if (access(ca2, F_OK) == 0) {
			curl_easy_setopt(curl, CURLOPT_CAINFO, ca2);
			pr_info(stdout, "curl_verify_cacert_pem: using %s", ca2);
		} else {
			pr_color(stdout, DOG_COL_GREEN,
				" * cURL: can't locate cacert.pem - "
				"SSL verification may fail.\n");
		...
		pr_info(stdout, "curl_verify_cacert_pem: Linux platform");
...
buf_init(struct buf* b)
{
	...
	pr_info(stdout, "buf_init: initializing buffer");
	...
	pr_info(stdout, "buf_init: buffer initialized with %d bytes", b->allocated);
} /* buf_init */
...
buf_free(struct buf* b)
{
	...
	pr_info(stdout, "buf_free: freeing buffer (len=%zu, allocated=%zu)", b->len, b->allocated);
...
write_callbacks(void* ptr, size_t size, size_t nmemb, void* userdata)
{
	...
	if (required > b->allocated) {
		...
		pr_info(stdout, "write_callbacks: reallocated to %zu bytes", new_alloc);
	} /* if */
...
memory_struct_init(struct memory_struct* mem)
{
	...
	pr_info(stdout, "memory_struct_init: initializing memory structure");
	...
	pr_info(stdout, "memory_struct_init: initialized with %d bytes", mem->allocated);
} /* memory_struct_init */
...
memory_struct_free(struct memory_struct* mem)
{
	...
	pr_info(stdout, "memory_struct_free: freeing memory (size=%zu, allocated=%zu)", 
	         mem->size, mem->allocated);
...
write_memory_callback(void* contents, size_t size, size_t nmemb, void* userp)
{
	...
	if (required > mem->allocated) {
		...
		pr_info(stdout, "write_memory_callback: reallocated to %zu bytes", new_alloc);
	} /* if */
