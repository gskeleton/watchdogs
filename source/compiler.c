...
_compiler_retrying:
		...
		*/
		...
		if (*compile_args_val == '\0' ||
			...
			int _garbage = !dogconfig.dog_garbage_access[DOG_GARBAGE_COMPILER_TARGET];
...
compiler_done2:
		...
		int ret = !dogconfig.dog_garbage_access[DOG_GARBAGE_COMPILER_RETRY];
		if (ret && this_proc_file != NULL &&
				...
				this_proc_file) != NULL)
				{
					if (strfind(compiler_mb, "error",
						...
						printf(DOG_COL_BCYAN
							...
							BKG_DEFAULT);
						sleep(3);
						dogconfig.dog_garbage_access
							[DOG_GARBAGE_COMPILER_RETRY] = DOG_GARBAGE_TRUE;
