...
{
  if (!dest || !path || !out || out_size < 1)
    return;
  
  /* Check destination path conditions */
  if (strlen(dest) < 1 || strcmp(dest, ".") == 0)
...
is_archive_file(const char* filename)
{
  ...
  if (_strend(filename, ".zip", true)) {
    return (true);
  } /* if */
  
  if (_strend(filename, ".tar", true)) {
    return (true);
  } /* if */
  
  if (_strend(filename, ".tar.gz", true)) {
    return (true);
  } /* if */
  
  if (_strend(filename, ".tgz", true)) {
    return (true);
  } /* if */
...
dog_extract_archive(const char* filename, const char* dir)
{
  ...
  if (_strend(filename, ".tar.gz", true) || _strend(filename, ".tgz", true)) {
    dog_extract_tar(filename, dir);
  } else if (_strend(filename, ".tar", true)) {
    dog_extract_tar(filename, dir);
  } else if (_strend(filename, ".zip", true)) {
