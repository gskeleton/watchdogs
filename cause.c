...
static const char* dog_find_warn_err(const char* line)
{
  ...
  char *found = NULL;
  size_t len = 0;
  for (c_idx = 0; ccs[c_idx].cs_t != NULL; ++c_idx) {
    char *cs_t = ccs[c_idx].cs_t;
    char *cs_i = ccs[c_idx].cs_i;
    found = strstr(line, cs_t);
    if (!found)
      continue;
    len = strlen(cs_t);
    if ((size_t)(found-line)+len <= strlen(line)) {
      return (cs_i);
    }
  } /* for */
...
static void pc_detailed(const char* out_put, int debug,
  ...
  long int stack_size, long int total_size)
{
  ...
  if (error_count < 1) {
    ...
    len = snprintf(outbuf, sizeof(outbuf),
      "Compilation Complete - OK! | " LR_CYAN "%d pass (warning) " LR_DEFAULT
      "| " LR_BLUE "%d fail (error)\n",
    ...
    /* Failed compilation */
    len = snprintf(outbuf, sizeof(outbuf),
      "Compilation Complete - Fail :( | " LR_CYAN "%d pass (warning) " LR_DEFAULT
      "| " LR_BLUE "%d fail (error)\n",
      warning_count, error_count);
  ...
  if (path_exists(out_put) && debug && error_count < 1) {
    /* Check if we have valid size information */
    if (header_size >= 1 && total_size >= 1) {
      /* Set file permissions */
      __set_default_access(out_put);
      
      /* Calculate file hash */
      unsigned long hash = crypto_djb2_hash_file(out_put);
...
void cause_pc_expl(const char* log_file, const char* out_put, int debug)
{
  FILE* _log_file = NULL;
  ...
  while (fgets(pc_line, sizeof(pc_line), _log_file)) {
    ...
    if (_strcase(pc_line, "Warnings.") ||
      _strcase(pc_line, "Warning.") ||
      _strcase(pc_line, "Errors.") ||
      _strcase(pc_line, "Error.")) {
      continue;
    } /* if */

    /* Parse header size information */
    if (_strcase(pc_line, "Header size:") ||
    ...
    else if (_strcase(pc_line, "Code size:") ||
    ...
    else if (_strcase(pc_line, "Data size:") ||
    ...
    else if (_strcase(pc_line, "Stack/heap size:") ||
    ...
    else if (_strcase(pc_line, "Total requirements:") ||
    ...
    else if (_strcase(pc_line, "Pawn Compiler ")) {
    ...
    if (_strcase(pc_line, "warning")) {
      ++warning_count;
    } /* if */
    
    if (_strcase(pc_line, "error")) {
      ++error_count;
    } /* if */
  ...
  pc_detailed(out_put, debug, warning_count, error_count,
